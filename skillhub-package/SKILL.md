...
5. **预判需求**：是否需要前测、是否需要动画（L2）、是否需要配音（L3）、是否需要开放任务、是否需要 AI 多模态互动区。

> ✅ **通过条件**：6 问全部有明确答案；学科、学段、课型、驱动模式已确定；L2/L3/互动区需求已标注。
...
# 浏览 references/data/ 目录，找到对应学科和领域的 JSON 文件，直接读取
```
...
| 🥈 | 直接读取 JSON 文件 | 脚本执行失败，但 `references/data/` 目录存在 | 用 `read_file` 工具读取 `references/data/{subject}/{domain}/_graph.json` 等文件 |
...
**3.3 L3 AI 语音讲解（Generation Gate 标注"需要"时执行）**：
...
> ✅ **通过条件**：L1 课件已完成；L2/L3 按 Gate 标注执行或跳过。
> ❌ **阻断**：课件中没有互动练习 = 不通过；没有前测或后测 = 不通过。
...
2. **L2/L3 显式决策**：
   > 如果 Generation Gate 中 L2 或 L3 标注为"不需要"，但课件中存在适合动画演示的内容（如物理实验过程、数学函数图像变化、化学反应过程），**必须主动提示用户**：
   >
   > "本课件中的 [具体内容] 适合用教学动画/语音讲解来增强效果。是否需要生成 L2 教学动画和/或 L3 AI 语音讲解？"

3. **输出文件清单**：明确列出本次生成的所有文件路径。

> ✅ **通过条件**：课件已交付；L2/L3 决策已完成（执行或已询问用户）；文件清单已列出。
...
| **L3** | AI 语音讲解 | 🔶 **显式决策** | 用户要求，或 Generation Gate 标注"需要" |
...
即使用户未要求 L2/L3，如果课件内容中存在以下情况，**必须在 Phase 4 交付时主动提示**：
...
| 任何知识点的分步讲解内容超过 500 字 | 建议 L3 语音讲解减轻阅读负担 |
| 课件模块数 ≥ 5 且内容量大 | 建议 L3 语音讲解提供导学 |

---

## 十三、15 条硬规则（违反任何一条 = Completeness Gate 不通过）
...
| **L3 — AI 语音讲解** | `public/tts/*.mp3` + `public/tts/*.srt` | Python 3.8+、edge-tts | ✅ AI 自动检测安装 | 🔶 显式触发 |

**L1 始终可独立运行。L2 和 L3 是渐进式增强。**
...
> **当 Generation Gate 标注 L3="需要"时，AI 必须自动完成安装和执行，用户无需手动操作。**
