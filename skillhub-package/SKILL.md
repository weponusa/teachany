...
**执行策略**：
1. **优先使用 Remotion（L2）**：如果 Generation Gate 标注 L2="需要"，则通过 Remotion 生成教学动画
2. **次选 AI 生视频工具**：如果环境中有视频生成工具可用，为关键过程生成短视频
3. **最低保障**：使用 CSS/JS 动画在 HTML 课件中实现简单过程动画（Canvas/SVG）

**视频嵌入规范**：
```html
<!-- AI 生成的教学短视频 -->
<div class="teachany-video-zone" data-context="物理·电解水·实验过程">
...
| **Agent B：英文课件** | 生成 `index_en.html` | 教学骨架 + HTML 骨架模板 + 学段 CSS | 完整英文课件 HTML | 默认执行（用户拒绝时跳过） |
...
Step 2：主 Agent 并行调用 task 工具分发任务
        ...
        ├── task(Agent B): "将以下中文教学骨架翻译为英文，生成 index_en.html..."
...
2. CSS 变量使用 {学段} 模板配色
3. 必选 section 不可删除：Hero、学习目标、前测、知识模块×{N}、综合任务、后测、小结
4. 每个模块遵循 6 块结构（ABT→讲解→深层理解→练习→纠错→小结）
5. 插图位置用 <img src="./assets/placeholder-{N}.png" alt="..."> 占位（Agent C 会生成实际图片）
6. 音频位置用 <audio src="./public/tts/seg{NN}_zh.mp3"> 占位（Agent D 会生成实际音频）
...
【Agent 协作模式】✅多 Agent 并行（A+B+C+D） / 🔶部分并行（___） / ❌单 Agent 串行（环境不支持 task 工具）
...
- **多 Agent 协作**：如果环境支持 `task` 工具，按 Section 10.5 的协作架构并行分发任务（Agent A 中文 + Agent B 英文 + Agent C 插图 + Agent D TTS）
...
> ✅ **通过条件**：L1 课件已完成（含双语版本）；L3 语音已生成（或用户明确拒绝）；L2 按 Gate 标注执行或跳过；课件已打包（Phase 3.5）。
> ❌ **阻断**：课件中没有互动练习 = 不通过；没有前测或后测 = 不通过；L3 未执行且用户未明确拒绝 = 不通过；未生成双语版本且用户未明确拒绝 = 不通过。
...
【总评】___ / 20 通过（#14 仅纯计算/纯习题/纯复习课可标 N/A；#15 用户拒绝时可标 N/A；#19 纯理科计算课可标 N/A）
【修复动作】（如有未通过项）___
═══════════════════════════════════════════
```

> ✅ **通过条件**：必检项（#1-#20）全部 ✅ 或 N/A（#14 仅纯计算/纯习题/纯复习课可 N/A；#15 用户拒绝时可 N/A；#19 纯理科计算课可 N/A），或未通过项已给出修复方案并执行修复。
> ❌ **阻断**：有 3 项以上 ❌ = 必须修复后再交付。
...
> ✅ **通过条件**：课件已交付（含双语）；L3 已完成或用户已拒绝；课件已打包；L2 决策已完成（执行或已询问用户）；文件清单已列出。
...
## 十三、23 条硬规则（违反任何一条 = Completeness Gate 不通过）
