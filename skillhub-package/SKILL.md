...
3. **判断课型**：新授 / 复习 / 专题 / 习题 / 实验 — 课型决定后续结构选择。
4. **预判需求**：是否需要前测、是否需要动画（L2）、是否需要配音（L3）、是否需要开放任务。

> ✅ **通过条件**：6 问全部有明确答案；学科、学段、课型已确定；L2/L3 需求已标注。
...
1. **ABT 引入设计**：为每个核心模块写 ABT 开场（And-But-Therefore 叙事）
...
4. **核心问题**：每个模块确定 1 个核心驱动问题（学生学完应该能回答的问题）

> ✅ **通过条件**：每个模块有 ABT 引入；知识点已列出并做了三分法标注；有前测设计；每个模块有核心问题。
> ❌ **阻断**：没有前测 = 不通过；ABT 引入缺失 = 不通过。
...
🔍 COMPLETENESS GATE — 输出完整性审查
═══════════════════════════════════════════
□ 1. ABT 引入：是否每个核心模块都有"为什么学"的叙事引入？
     → ✅/❌ ___
...
## 十三、13 条硬规则（违反任何一条 = Completeness Gate 不通过）

> 以下规则在整个课件开发流程中**始终有效**，不可因"简化""省事""上下文不够"等理由绕过。

| # | 硬规则 | 违反后果 |
|:---|:---|:---|
| 1 | **先判断课型**（新授/复习/习题/专题/实验），再选结构。不可假设所有任务都是"新授课"。 | 课型错误 → 整体结构需要重做 |
