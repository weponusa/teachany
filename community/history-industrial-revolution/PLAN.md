...
- **课件 ID**：history-industrial-revolution
- **node_id**：history-industrial-revolution
- **学段/学科**：history·9年级（初中历史九年级上册）
- **ABT 叙事**：已注入（见课件 #why-learn 区块）
- **Bloom 层级覆盖**：L1-L4 多层覆盖
- **ConcepTest 锚点**：英国条件+关键发明+社会影响
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| M1 | Hero 知识结构图 | 工业革命整体结构 | Hero SVG | assets/hero-infographic.svg | 内嵌 SVG | test -f assets/hero-infographic.svg |
| M2 | ABT 情境引入 | 已知/问题/新知 | 卡片式 HTML | inline#why-learn | HTML 内嵌 | grep -q why-learn index.html |
| M3 | 核心内容模块 | 英国条件+关键发明+社会影响 | 信息卡+互动 | inline | HTML 内嵌 | node scripts/validate-courseware.cjs <dir> |
| M4 | 互动仿真 | 无 | Canvas 动画 | inline（无外部仿真） | N/A | curl -I https://www.geogebra.org/apps/deployggb.js |
| M5 | 知识图谱 | 前置后续导航 | 标准公共模块 | data-teachany-kg="history-industrial-revolution" | python3 scripts/build-teachany-kg-manifest.py | python3 scripts/check-knowledge-graph.py <dir> |
...
- [x] **知识图谱**：`data-teachany-kg="history-industrial-revolution"` 已列入 M5
