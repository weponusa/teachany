...
- **node_id**：chn-compound-vowel
- **学段/学科**：chinese·1年级（小学语文一年级）
- **ABT 叙事**：已注入（见课件 #why-learn 区块）
- **Bloom 层级覆盖**：L1-L4 多层覆盖
- **ConcepTest 锚点**：9个复韵母发音+拼读规则
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| M1 | Hero 知识结构图 | 复韵母整体结构 | Hero SVG | assets/hero-infographic.svg | 内嵌 SVG | test -f assets/hero-infographic.svg |
| M2 | ABT 情境引入 | 已知/问题/新知 | 卡片式 HTML | inline#why-learn | HTML 内嵌 | grep -q why-learn index.html |
| M3 | 核心内容模块 | 9个复韵母发音+拼读规则 | 信息卡+互动 | inline | HTML 内嵌 | node scripts/validate-courseware.cjs <dir> |
| M4 | 互动仿真 | 无 | Canvas 动画 | inline（无外部仿真） | N/A | curl -I https://www.geogebra.org/apps/deployggb.js |
| M5 | 知识图谱 | 前置后续导航 | 标准公共模块 | data-teachany-kg="chn-compound-vowel" | python3 scripts/build-teachany-kg-manifest.py | python3 scripts/check-knowledge-graph.py <dir> |
...
- [x] **知识图谱**：`data-teachany-kg="chn-compound-vowel"` 已列入 M5
