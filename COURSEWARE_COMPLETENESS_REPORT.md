...
**验证结果**：✅ 两个区域都正确集成，用户可以看到全部 131 个课件
...
所有 131 个原课件都有有效的 `node_id`，全部保留在系统中（官方 7 个 + 社区 82 个）。
