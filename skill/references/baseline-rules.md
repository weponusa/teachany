...
### ⑦ Knowledge graph section (use STANDARD MODULE API, not hand-written HTML)

**What**: A graph showing prerequisites + current course + next courses, rendered by the standard module `teachany-knowledge-graph.js` from `teachany-kg-manifest.json`.

**HARD RULE — use the standard API, not hand-written HTML**:

```html
<section data-tsh="知识图谱 - 看这一课在历史脉络中的位置">
  <div data-teachany-kg="hist-m-renaissance">
    <canvas class="tkg-fallback-canvas" width="720" height="120"></canvas>
  </div>
</section>
```

That's it. The standard module:
1. Reads `teachany-kg-manifest.json` (auto-generated by `rebuild-index.py`)
2. Builds the graph: center node (current course) + prerequisites + next + same-domain neighbors + edges
3. Renders SVG with the SAME visual language as `tree.html` (the global knowledge map)
4. Adds hover tooltips, click-to-jump, dashed boxes for nodes without coursewares
5. Falls back to canvas drawing if the manifest fetch fails

⛔ **Anti-pattern — DO NOT hand-write HTML like this**:

```html
<!-- ❌ WRONG: hand-writing kg-container/kg-row/kg-node tags  -->
<section id="knowledge-graph">
  <h2>🌐 知识图谱：本课的位置</h2>
  <div class="kg-container">
    <div class="kg-row kg-prereqs">
      <span class="kg-label">前置：</span>
      <a href="../hist-m-medieval-europe/" class="kg-node">中世纪欧洲</a>
    </div>
    <!-- ... -->
  </div>
</section>
...
**Why hand-writing is wrong**:
- Visual style **drifts from `tree.html`** → inconsistent UX across coursewares
- Doesn't pick up upstream module updates (when standard module gets new node types, animation, hover details, your hand-written HTML stays static)
- Doesn't sync with knowledge tree changes (if `kg-manifest.json` updates a node's name or adds a new prereq, your hand-written HTML is wrong)
- Hard-coded relative URLs `../<course-id>/` break when courseware location changes

**Verify**:
```js
// In browser console after page load:
document.querySelector('[data-teachany-kg]')   // → element
document.querySelector('.tkg-svg')             // → SVG rendered by module
// If both are non-null, module is working.
```

If the manifest fetch fails (e.g., local dev without dev server), the module renders a canvas fallback automatically. Don't hand-write to "fix" the fallback — fix the network/path.

---

### ⑧ AI tutor card (use STANDARD MODULE API, not hand-written HTML)

**What**: A clickable card embedded in the page that opens the AI tutor FAB panel. Required because students often miss the floating left-bottom 💡 ball.

**HARD RULE — use the standard API**:

```html
<section data-tsh="AI 学伴 - 课后答疑可问任何问题">
  <div data-teachany-tutor-card></div>
</section>
```

That's it. The standard module `teachany-tutor-card.js`:
1. Reads `<title>` and infers the subject name (the part before `·`)
2. Reads `course-subject` meta and adapts suggestion questions
3. Renders icon (💡) + title (`AI 学伴 · 关于「<topic>」随时问`) + description + 4 contextual suggestion bubbles
4. Wires click handlers to `ai-tutor.js`'s FAB
5. Provides "立即提问" + "配置 API" buttons

⛔ **Anti-pattern — DO NOT hand-write the card HTML**:

```html
<!-- ❌ WRONG: hand-writing card content  -->
<section id="ai-tutor-card">
  <div class="custom-card">
    <h3>💡 AI 学伴</h3>
    <p>这里是 AI 助教，有问题随时问...</p>
    <button onclick="openTutor()">提问</button>
  </div>
</section>
```

**Why**: Hand-writing breaks subject awareness, theme inheritance, suggestion-question intelligence, and FAB-binding logic. The standard module is **already smarter than what you'd manually write**.

---

### ⑨–⑫ Other 4 of the Five-piece suite

All five (`ai-tutor` + `tutor-card` + `tts-narrator` + `section-hints` + `knowledge-graph`) must be loaded:
...
Plus standard module API placeholders (NOT hand-written content):
- `<div data-teachany-tutor-card></div>` (for ⑧)
- `<div data-teachany-kg="<node_id>"><canvas class="tkg-fallback-canvas" width="720" height="120"></canvas></div>` (for ⑦)
- `data-tsh` attributes on each main `<section>` (for section-hints)
- `data-tts` attributes on key paragraphs (for tts-narrator)

Use `python3 scripts/apply-standard-modules.py --only community/<course-id>` to batch-inject these placeholders.
...
window.__TeachAnyTutorCardInit          // → true (mounted)
```
...
### ⑮ Top brand bar (TeachAny logo + Gallery link + DUAL VERSION)

**What**: A pinned top row above or at top of the hero section, containing TeachAny identity + navigation + **TWO versions** (course version AND skill version).

**Why**: Students landing on a courseware URL need four things immediately — (a) what platform this is, (b) how to navigate to other lessons, (c) **what courseware version** (for content updates / bug reports), (d) **what skill version** (for capability/format updates).

**Showing only one version is a violation**:
- Only course version → students don't know if the courseware uses latest TeachAny capabilities
- Only skill version → students don't know if the content has been updated
...
<div class="teachany-brand-bar">
  ...
  <div class="brand-right">
    ...
    <span class="brand-version" title="课件版本 / TeachAny skill 版本">
      v<span id="course-version-display">1.0.0</span>
      <span class="brand-skill-version">·skill <span id="skill-version-display">v7.10.1</span></span>
    </span>
  </div>
</div>
<style>
...
.brand-version { color: #d1c4a3; font-family: monospace; font-size: 0.85em; padding: 2px 8px; background: rgba(255,255,255,0.1); border-radius: 10px; }
.brand-skill-version { color: #968358; font-size: 0.92em; margin-left: 4px; }
</style>
<script>
// Auto-sync from meta tags
document.addEventListener('DOMContentLoaded', () => {
  // course-version → display element
  ...
  // teachany-version → display element (skill 版本)
  ...
  if (sv && svEl) svEl.textContent = 'v' + sv;
});
</script>
```

**Required `<head>` meta tags for both versions**:
```html
<meta name="course-version" content="1.0.0">      <!-- 课件版本 -->
<meta name="teachany-version" content="7.10.1">   <!-- skill 版本（不带 v 前缀，JS 会加） -->
```
...
# Both version displays present
grep -c 'course-version-display' index.html                       # Must be ≥ 1
grep -c 'skill-version-display' index.html                        # Must be ≥ 1

# Both meta tags present
grep -cE 'name="course-version"|name="teachany-version"' index.html  # Must be 2
```
...
#### 🛰️ Projection-alignment hard rule (投影对齐铁律)

**The library hillshade is a global Plate Carrée image** (4096×2048, equirectangular projection covering -180° to 180° longitude × -90° to 90° latitude).

**HARD RULE**: when using `L.imageOverlay()`, the bounds parameter tells Leaflet "this image spans these geographic coordinates". For a global image, you MUST pass global bounds:

```js
// ✅ CORRECT — global bounds + setView/fitBounds for region
const globalBounds = [[-90, -180], [90, 180]];
const europeBounds = [[35, -10], [60, 30]];
L.imageOverlay('./assets/maps/hillshade.jpg', globalBounds, { opacity: 0.85 }).addTo(map);
map.fitBounds(europeBounds);  // Zoom to Europe; image still projects globally

// ❌ WRONG — telling Leaflet "this global image spans only Europe"
L.imageOverlay('./assets/maps/hillshade.jpg', [[35, -10], [60, 30]]).addTo(map);
// Result: the entire world (4096×2048 pixels of global terrain) is stretched into the
// Europe rectangle. Alps appear in Africa. Madrid floats in the Atlantic. Boundaries
// on the GeoJSON layer are correct (real lat/lng), but the hillshade is offset.
```

**How to debug a projection-alignment issue**:
1. Toggle the hillshade layer off — does the GeoJSON look correct? (Yes? Then hillshade is wrong.)
2. Inspect `imageOverlay` bounds in the source — does it cover the FULL image extent or just a region?
3. If the image is a global Plate Carrée (most library hillshades), bounds must be global.
4. If you specifically need a regional crop, **pre-crop the image** with `gdal_translate` to match the region exactly:
   ```bash
   gdal_translate -projwin -10 60 30 35 global-color-hillshade-4k.jpg europe-hillshade.jpg
   # Then bounds = [[35, -10], [60, 30]] is correct because the image is now just Europe
   ```

**Library hillshade extent reference**:

| Library file | Pixel size | Geographic extent | Correct bounds |
|:---|:---:|:---|:---|
| `physical/hillshade/global-hillshade-4k.jpg` | 4096×2048 | Global Plate Carrée | `[[-90,-180],[90,180]]` |
| `physical/hillshade/global-color-hillshade-4k.jpg` | 4096×2048 | Global Plate Carrée | `[[-90,-180],[90,180]]` |
| `physical/hillshade/global-hillshade-2k.jpg` | 2048×1024 | Global Plate Carrée | `[[-90,-180],[90,180]]` |
| Custom regional crop (you make it) | varies | Whatever you cropped | match exactly |

⛔ **Anti-pattern**: assuming "the hillshade is for Europe so bounds should be Europe" — bounds describes the IMAGE extent, not your map view. Use `setView` / `fitBounds` to control what part of the (correctly-projected) image the user sees.

---

**Recommended implementation** (Leaflet + library hillshade with correct projection):
...
<script>
const map = L.map('map').setView([46, 10], 5);  // Europe center, zoom 5

// Layer 1: Hillshade terrain (library is GLOBAL Plate Carrée — bounds MUST be global)
const globalBounds = [[-90, -180], [90, 180]];
const europeBounds = [[35, -10], [60, 30]];
L.imageOverlay('./assets/maps/hillshade.jpg', globalBounds, { opacity: 0.85 }).addTo(map);
map.fitBounds(europeBounds);  // Zoom user view to Europe (image still projects globally)
