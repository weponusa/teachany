# 免疫 / Immunity

<!-- TeachAny KP: kp_id=kp-biology-cam-al-immunity-al node_id=biology-cam-al-immunity-al subject=biology stage=al -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-biology-cam-al-immunity-al` |
| node_id | `biology-cam-al-immunity-al` |
| 知识点 | 免疫 |
| 英文名 | Immunity |
| 学科 | 生物 (biology) |
| 学段 | Cambridge A-Level（高中） (al) |
| 年级 | 11 |
| 课标系统 | Cambridge 剑桥国际 |
| 领域 | AS · 运输与免疫 / AS · Transport & Immunity / AS · Transport & Immunity |
| tree 源 | `data/trees/cambridge/al/biology.json` |

## 课标原文（Cambridge Syllabus）

### 课标原文

1 describe the structure of a DNA molecule as a double helix, including:
• the importance of complementary base pairing between
the 5′ to 3′ strand and the 3′ to 5′ strand (antiparallel
strands)
• differences in hydrogen bonding between C–G and A–T
base pairs
• linking of nucleotides by phosphodiester bonds

2 state that the bases adenine and guanine are purines
with a double ring structure, and that the bases cytosine,
thymine and uracil are pyrimidines with a single ring structure
(structural formulae for bases are not expected)

1 describe the structure of nucleotides, including the
phosphorylated nucleotide ATP (structural formulae are not
expected)

## 知识图谱关系

### 前驱知识

- `biology-cam-al-infectious-disease`

## 课标摘要

1 describe the structure of a DNA molecule as a double helix, including: • the importance of complementary base pairing between the 5′ to 3′ strand and the 3′ to 5′ strand (antiparallel strands) • differences in hydrogen bonding between C–G and A–T base pairs • linking of nucleotides by phosphodiester bonds；2 state that the bases adenine and guanine are purines with a double ring structure, and that the bases cytosine, thymine and uracil are pyrimidines with a single ring structure (structural formulae for bases are not expected)；1 describe the structure of nucleotides, including the phosphorylated nucleotide ATP (structural formulae are not expected)
