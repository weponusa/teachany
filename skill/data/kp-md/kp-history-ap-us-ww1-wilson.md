# 一战与威尔逊 / WWI & Wilson

<!-- TeachAny KP: kp_id=kp-history-ap-us-ww1-wilson node_id=history-ap-us-ww1-wilson subject=us-history stage=high -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-history-ap-us-ww1-wilson` |
| node_id | `history-ap-us-ww1-wilson` |
| 知识点 | 一战与威尔逊 |
| 英文名 | WWI & Wilson |
| 学科 | 美国历史 (us-history) |
| 学段 | 高中 (high) |
| 年级 | 12 |
| 课标系统 | AP 美国大学先修 |
| 领域 | Period 6-7 镀金时代与进步时代 Gilded Age & Progressive (1865-1945) |
| tree 源 | `data/trees/ap/high/us-history.json` |

## 课标原文（AP Course Framework）

### 课标原文

Unit 1: Learning Objective B: Explain how and why various native populations in the period before European contact interacted with the natural environment in North America.

KC-1.1.I: Different native societies adapted to and transformed their environments through innovations in agriculture, resource use, and social structure.

KC-1.1.I.C: In the Northeast, the Mississippi River Valley, and along the Atlantic seaboard, some societies developed mixed agricultural and hunter-gatherer economies that favored the development of permanent villages.

KC-1.1.I.D: Societies in the Northwest and present-day California supported themselves by hunting and gathering, and in some areas developed settled communities supported by the vast resources of the ocean.

## 知识图谱关系

### 前驱知识

- `history-ap-us-p7-progressive`

## 课标摘要

Unit 1: Learning Objective B: Explain how and why various native populations in the period before European contact interacted with the natural environment in North America.；KC-1.1.I: Different native societies adapted to and transformed their environments through innovations in agriculture, resource use, and social structure.；KC-1.1.I.C: In the Northeast, the Mississippi River Valley, and along the Atlantic seaboard, some societies developed mixed agricultural and hunter-gatherer economies that favored the development of permanent villages.；KC-1.1.I.D: Societies in the Northwest and present-day California supported themselves by hunting and gathering, and in some areas developed settled communities supported by the vast resources of the ocean.
