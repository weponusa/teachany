# C1 光合作用（光反应/卡尔文循环） / C1 Photosynthesis

<!-- TeachAny KP: kp_id=kp-bio-ib-dp-photosynthesis node_id=bio-ib-dp-photosynthesis subject=biology stage=dp -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-bio-ib-dp-photosynthesis` |
| node_id | `bio-ib-dp-photosynthesis` |
| 知识点 | C1 光合作用（光反应/卡尔文循环） |
| 英文名 | C1 Photosynthesis |
| 学科 | 生物 (biology) |
| 学段 | IB-DP（高中） (dp) |
| 年级 | 11 |
| 课标系统 | IB 国际文凭 |
| 领域 | 主题 C · 互动与相互依存 Interaction and Interdependence |
| tree 源 | `data/trees/ib/dp/biology.json` |

## 课标原文（IB Programme）

### 课标原文

A1.2.1—DNA as the genetic material of all living organisms
Some viruses use RNA as their genetic material but viruses are not considered to be living.

A1.2.6—DNA as a double helix made of two antiparallel strands of nucleotides with two strands linked by hydrogen bonding between complementary base pairs
In diagrams of DNA structure, students should draw the two strands antiparallel, but are not required to draw the helical shape. Students should show adenine (A) paired with thymine (T), and guanine (G) paired with cytosine (C). Students are not required to memorize the relative lengths of the purine and pyrimidine bases, or the numbers of hydrogen bonds.

A1.2.8—Role of complementary base pairing in allowing genetic information to be replicated and expressed
Students should understand that complementarity is based on hydrogen bonding.

A1.2.9—Diversity of possible DNA base sequences and the limitless capacity of DNA for storing information
Explain that diversity by any length of DNA molecule and any base sequence is possible. Emphasize the enormous capacity of DNA for storing data with great economy.

## 知识图谱关系

### 前驱知识

- `bio-ib-dp-enzymes`

## 课标摘要

A1.2.1—DNA as the genetic material of all living organisms Some viruses use RNA as their genetic material but viruses are not considered to be living.；A1.2.6—DNA as a double helix made of two antiparallel strands of nucleotides with two strands linked by hydrogen bonding between complementary base pairs In diagrams of DNA structure, students should draw the two strands antiparallel, but are not required to draw the helical shape. Students should show adenine (A) paired with thymine (T), and guanine (G) paired with cytosine (C). Students are not required to memorize the relative lengths of the purine and pyrimidine bases, or the numbers of hydrogen bonds.；A1.2.8—Role of complementary base pairing in allowing genetic information to be replicated and expressed Students should understand that complementarity is based on hydrogen bonding.；A1.2.9—Diversity of possible DNA base sequences and the limitless capacity of DNA for storing information Explain that diversity by any length of DNA molecule and any base sequence is possible. Emphasize the enormous capacity of DNA for storing data with great economy.
