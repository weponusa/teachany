# 有丝分裂 / Mitotic cycle

<!-- TeachAny KP: kp_id=kp-biology-cam-al-mitotic-cycle node_id=biology-cam-al-mitotic-cycle subject=biology stage=al -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-biology-cam-al-mitotic-cycle` |
| node_id | `biology-cam-al-mitotic-cycle` |
| 知识点 | 有丝分裂 |
| 英文名 | Mitotic cycle |
| 学科 | 生物 (biology) |
| 学段 | Cambridge A-Level（高中） (al) |
| 年级 | 11 |
| 课标系统 | Cambridge 剑桥国际 |
| 领域 | AS · 细胞与分子 / AS · Cells & Molecules / AS · Cells & Molecules |
| tree 源 | `data/trees/cambridge/al/biology.json` |

## 课标原文（Cambridge Syllabus）

### 课标原文

describe the formation of a glycosidic bond by condensation, with reference to disaccharides, including sucrose, and polysaccharides

describe the breakage of a glycosidic bond in polysaccharides and disaccharides by hydrolysis, with reference to the non-reducing sugar test

describe and carry out a test to identify the presence of non-reducing sugars, using acid hydrolysis and Benedict’s solution

## 知识图谱关系

### 前驱知识

- `biology-cam-al-cell-structure-al`

## 课标摘要

describe the formation of a glycosidic bond by condensation, with reference to disaccharides, including sucrose, and polysaccharides；describe the breakage of a glycosidic bond in polysaccharides and disaccharides by hydrolysis, with reference to the non-reducing sugar test；describe and carry out a test to identify the presence of non-reducing sugars, using acid hydrolysis and Benedict’s solution
