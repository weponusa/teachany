# 语言分析 / Language analysis

<!-- TeachAny KP: kp_id=kp-english-cam-igcse-language-analysis node_id=english-cam-igcse-language-analysis subject=english stage=igcse -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-english-cam-igcse-language-analysis` |
| node_id | `english-cam-igcse-language-analysis` |
| 知识点 | 语言分析 |
| 英文名 | Language analysis |
| 学科 | 英语 (english) |
| 学段 | Cambridge-IGCSE（初中/高中） (igcse) |
| 年级 | 10 |
| 课标系统 | Cambridge 剑桥国际 |
| 领域 | 阅读理解 / Reading / Reading |
| tree 源 | `data/trees/cambridge/igcse/english.json` |

## 课标原文（Cambridge Syllabus）

### 课标原文

• develop the ability to communicate clearly, accurately and effectively when speaking and writing

• learn how to use a wide range of vocabulary, and the correct grammar, spelling and punctuation

• develop a personal style and an awareness of the audience being addressed.

## 知识图谱关系

### 前驱知识

- `english-cam-igcse-inference-igc`

## 课标摘要

• develop the ability to communicate clearly, accurately and effectively when speaking and writing；• learn how to use a wide range of vocabulary, and the correct grammar, spelling and punctuation；• develop a personal style and an awareness of the audience being addressed.
