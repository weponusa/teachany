# 法拉第电磁感应 / Faraday's law

<!-- TeachAny KP: kp_id=kp-physics-ap-c-faraday-law node_id=physics-ap-c-faraday-law subject=physics-c stage=high -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-physics-ap-c-faraday-law` |
| node_id | `physics-ap-c-faraday-law` |
| 知识点 | 法拉第电磁感应 |
| 英文名 | Faraday's law |
| 学科 | 物理 C (physics-c) |
| 学段 | 高中 (high) |
| 年级 | 12 |
| 课标系统 | AP 美国大学先修 |
| 领域 | 电磁-磁场与电磁感应 E&M: Magnetism |
| tree 源 | `data/trees/ap/high/physics-c.json` |

## 课标原文（AP Course Framework）

### 课标原文

2.5.A.2
Newton’s second law of motion states that the acceleration of a system’s center of mass has a magnitude proportional to the magnitude of the net force exerted on the system and is in the same direction as that net force.
Relevant equation:
∑F
F
net
a = = 
sys m m
sys sys

2.5.A.3
The velocity of a system’s center of mass will only change if a nonzero net external force is exerted on that system.

## 知识图谱关系

### 前驱知识

- `physics-ap-c-ampere-law`

## 课标摘要

2.5.A.2 Newton’s second law of motion states that the acceleration of a system’s center of mass has a magnitude proportional to the magnitude of the net force exerted on the system and is in the same direction as that net force. Relevant equation: ∑F F net a = =  sys m m sys sys；2.5.A.3 The velocity of a system’s center of mass will only change if a nonzero net external force is exerted on that system.
