# 条件分支 / if / else-if / else

<!-- TeachAny KP: kp_id=kp-cs-ap-a-if-else node_id=cs-ap-a-if-else subject=cs stage=high -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-cs-ap-a-if-else` |
| node_id | `cs-ap-a-if-else` |
| 知识点 | 条件分支 |
| 英文名 | if / else-if / else |
| 学科 | 计算机科学 (cs) |
| 学段 | 高中 (high) |
| 年级 | 10 |
| 课标系统 | AP 美国大学先修 |
| 领域 | Unit 3-4 控制流 Control Flow |
| tree 源 | `data/trees/ap/high/cs.json` |

## 课标原文（AP Course Framework）

### 课标原文

1.3.C.2
The arithmetic operators consist of addition +, subtraction -, multiplication *, division /, and remainder %. An arithmetic operation that uses two int values will evaluate to an int value. An arithmetic operation that uses at least one double value will evaluate to a double value.

1.3.C.5
Operators can be used to construct compound expressions. At compile time, numeric values are associated with operators according to operator precedence to determine how they are grouped. Parentheses can be used to modify operator precedence. Multiplication, division, and remainder have precedence over addition and subtraction. Operators with the same precedence are evaluated from left to right.

1.3.C.3
When dividing numeric values that are both int values, the result is only the integer portion of the quotient. When dividing numeric values that use at least one double value, the result is the quotient.

1.3.C.4
The remainder operator % is used to compute the remainder when one number a is divided by another number b.

## 知识图谱关系

### 前驱知识

- `cs-ap-a-boolean-logic`

## 课标摘要

1.3.C.2 The arithmetic operators consist of addition +, subtraction -, multiplication *, division /, and remainder %. An arithmetic operation that uses two int values will evaluate to an int value. An arithmetic operation that uses at least one double value will evaluate to a double value.；1.3.C.5 Operators can be used to construct compound expressions. At compile time, numeric values are associated with operators according to operator precedence to determine how they are grouped. Parentheses can be used to modify operator precedence. Multiplication, division, and remainder have precedence over addition and subtraction. Operators with the same precedence are evaluated from left to right.；1.3.C.3 When dividing numeric values that are both int values, the result is only the integer portion of the quotient. When dividing numeric values that use at least one double value, the result is the quotient.；1.3.C.4 The remainder operator % is used to compute the remainder when one number a is divided by another number b.
