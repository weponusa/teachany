# 水的性质 / Properties of water

<!-- TeachAny KP: kp_id=kp-biology-ap-water-properties node_id=biology-ap-water-properties subject=biology stage=high -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-biology-ap-water-properties` |
| node_id | `biology-ap-water-properties` |
| 知识点 | 水的性质 |
| 英文名 | Properties of water |
| 学科 | 生物 (biology) |
| 学段 | 高中 (high) |
| 年级 | 10 |
| 课标系统 | AP 美国大学先修 |
| 领域 | Unit 1-2 分子与细胞 Molecular & Cell |
| tree 源 | `data/trees/ap/high/biology.json` |

## 课标原文（AP Course Framework）

### 课标原文

BIG IDEA 1: EVOLUTION
The process of evolution drives the diversity and unity of life. Evolution is a change in the genetic makeup of a population over time, with natural selection as its major driving mechanism. Darwin’s theory, which is supported by evidence from many scientific disciplines, states that inheritable variations occur in individuals in a population. Due to competition for limited resources, individuals with more favorable genetic variations are more likely to survive and produce more offspring, thus passing traits to future generations.

7.2 Natural Selection
Required Course Content
BIG IDEA 1
Evolution: The process of evolution drives the diversity and unity of life.
LEARNING OBJECTIVE
7.2.A
Explain how natural selection affects populations.
ESSENTIAL KNOWLEDGE
7.2.A.1
Natural selection is a major mechanism of evolution. Natural selection acts on phenotypic variations in populations.

ESSENTIAL KNOWLEDGE
7.2.A.2
Individuals with more favorable phenotypes are more likely to survive and produce more offspring, thus passing traits to subsequent generations.

ESSENTIAL KNOWLEDGE
7.2.A.3
Evolutionary fitness is measured by reproductive success.

## 课标摘要

BIG IDEA 1: EVOLUTION The process of evolution drives the diversity and unity of life. Evolution is a change in the genetic makeup of a population over time, with natural selection as its major driving mechanism. Darwin’s theory, which is supported by evidence from many scientific disciplines, states that inheritable variations occur in individuals in a population. Due to competition for limited resources, individuals with more favorable genetic variations are more likely to survive and produce more offspring, thus passing traits to future generations.；7.2 Natural Selection Required Course Content BIG IDEA 1 Evolution: The process of evolution drives the diversity and unity of life. LEARNING OBJECTIVE 7.2.A Explain how natural selection affects populations. ESSENTIAL KNOWLEDGE 7.2.A.1 Natural selection is a major mechanism of evolution. Natural selection acts on phenotypic variations in populations.；ESSENTIAL KNOWLEDGE 7.2.A.2 Individuals with more favorable phenotypes are more likely to survive and produce more offspring, thus passing traits to subsequent generations.；ESSENTIAL KNOWLEDGE 7.2.A.3 Evolutionary fitness is measured by reproductive success.
