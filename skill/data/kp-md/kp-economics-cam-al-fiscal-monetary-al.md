# 财政与货币政策 / Fiscal & monetary policy

<!-- TeachAny KP: kp_id=kp-economics-cam-al-fiscal-monetary-al node_id=economics-cam-al-fiscal-monetary-al subject=economics stage=al -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-economics-cam-al-fiscal-monetary-al` |
| node_id | `economics-cam-al-fiscal-monetary-al` |
| 知识点 | 财政与货币政策 |
| 英文名 | Fiscal & monetary policy |
| 学科 | 经济学 (economics) |
| 学段 | Cambridge A-Level（高中） (al) |
| 年级 | 12 |
| 课标系统 | Cambridge 剑桥国际 |
| 领域 | 宏观经济 / Macroeconomics / Macroeconomics |
| tree 源 | `data/trees/cambridge/al/economics.json` |

## 课标原文（Cambridge Syllabus）

### 课标原文

2.1 Demand and supply curves
2.1.1 effective demand
2.1.2 individual and market demand and supply
2.1.3 determinants of demand
2.1.4 determinants of supply
2.1.5 causes of a shift in the demand curve (D)
2.1.6 causes of a shift in the supply curve (S)
2.1.7 distinction between the shift in the demand or supply curve and the movement along these curves

2.4 The interaction of demand and supply
2.4.1 definition of market equilibrium and disequilibrium
2.4.2 effects of shifts in demand and supply curves on equilibrium price and quantity

2.4.4 functions of price in resource allocation; rationing, signalling (transmission of preferences) and incentivising

## 知识图谱关系

### 前驱知识

- `economics-cam-al-ad-as-al`

## 课标摘要

2.1 Demand and supply curves 2.1.1 effective demand 2.1.2 individual and market demand and supply 2.1.3 determinants of demand 2.1.4 determinants of supply 2.1.5 causes of a shift in the demand curve (D) 2.1.6 causes of a shift in the supply curve (S) 2.1.7 distinction between the shift in the demand or supply curve and the movement along these curves；2.4 The interaction of demand and supply 2.4.1 definition of market equilibrium and disequilibrium 2.4.2 effects of shifts in demand and supply curves on equilibrium price and quantity；2.4.4 functions of price in resource allocation; rationing, signalling (transmission of preferences) and incentivising
