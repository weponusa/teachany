# 冲突与和平 / Conflict & peace

<!-- TeachAny KP: kp_id=kp-humanities-cam-igcse-conflict-gp node_id=humanities-cam-igcse-conflict-gp subject=global-persp stage=igcse -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-humanities-cam-igcse-conflict-gp` |
| node_id | `humanities-cam-igcse-conflict-gp` |
| 知识点 | 冲突与和平 |
| 英文名 | Conflict & peace |
| 学科 | 全球视野 (global-persp) |
| 学段 | Cambridge-IGCSE（初中/高中） (igcse) |
| 年级 | 10 |
| 课标系统 | Cambridge 剑桥国际 |
| 领域 | 全球议题 / Global Issues / Global Issues |
| tree 源 | `data/trees/cambridge/igcse/global-persp.json` |

## 课标原文（Cambridge Syllabus）

### 课标原文

The aims are to enable students to:
• develop the ability to evaluate claims and evidence
• make independent judgements supported by reasoning and evidence, and understand the importance of justifying their own opinions

Learners will develop skills of independent enquiry and learn about appropriate research methods and methodology.

AO1 Research, analysis and evaluation
• identify and analyse arguments, evidence and perspectives
• synthesise and evaluate arguments, evidence and perspectives

AO2 Reflection
• acknowledge different perspectives and evaluate their impact on the learner’s own standpoint

## 课标摘要

The aims are to enable students to: • develop the ability to evaluate claims and evidence • make independent judgements supported by reasoning and evidence, and understand the importance of justifying their own opinions；Learners will develop skills of independent enquiry and learn about appropriate research methods and methodology.；AO1 Research, analysis and evaluation • identify and analyse arguments, evidence and perspectives • synthesise and evaluate arguments, evidence and perspectives；AO2 Reflection • acknowledge different perspectives and evaluate their impact on the learner’s own standpoint
