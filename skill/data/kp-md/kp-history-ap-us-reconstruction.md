# 重建时期 / Reconstruction

<!-- TeachAny KP: kp_id=kp-history-ap-us-reconstruction node_id=history-ap-us-reconstruction subject=us-history stage=high -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-history-ap-us-reconstruction` |
| node_id | `history-ap-us-reconstruction` |
| 知识点 | 重建时期 |
| 英文名 | Reconstruction |
| 学科 | 美国历史 (us-history) |
| 学段 | 高中 (high) |
| 年级 | 11 |
| 课标系统 | AP 美国大学先修 |
| 领域 | Period 4-5 扩张与内战 Expansion & Civil War (1800-1877) |
| tree 源 | `data/trees/ap/high/us-history.json` |

## 课标原文（AP Course Framework）

### 课标原文

Explain the causes and effects of the innovations in technology, agriculture, and commerce over time.

Innovations including textile machinery, steam engines, interchangeable parts, the telegraph, and agricultural inventions increased the efficiency of production methods.

Legislation and judicial systems supported the development of roads, canals, and railroads, which extended and enlarged markets and helped foster regional interdependence. Transportation networks linked the North and Midwest more closely than they linked regions in the South.

Increasing Southern cotton production and the related growth of Northern manufacturing, banking, and shipping industries promoted the development of national and international commercial ties.

## 知识图谱关系

### 前驱知识

- `history-ap-us-p5-civil-war`

## 课标摘要

Explain the causes and effects of the innovations in technology, agriculture, and commerce over time.；Innovations including textile machinery, steam engines, interchangeable parts, the telegraph, and agricultural inventions increased the efficiency of production methods.；Legislation and judicial systems supported the development of roads, canals, and railroads, which extended and enlarged markets and helped foster regional interdependence. Transportation networks linked the North and Midwest more closely than they linked regions in the South.；Increasing Southern cotton production and the related growth of Northern manufacturing, banking, and shipping industries promoted the development of national and international commercial ties.
