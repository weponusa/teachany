# 金属提取 / Metals & extraction

<!-- TeachAny KP: kp_id=kp-chemistry-cam-igcse-metals-extraction node_id=chemistry-cam-igcse-metals-extraction subject=chemistry stage=igcse -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-chemistry-cam-igcse-metals-extraction` |
| node_id | `chemistry-cam-igcse-metals-extraction` |
| 知识点 | 金属提取 |
| 英文名 | Metals & extraction |
| 学科 | 化学 (chemistry) |
| 学段 | Cambridge-IGCSE（初中/高中） (igcse) |
| 年级 | 10 |
| 课标系统 | Cambridge 剑桥国际 |
| 领域 | 有机化学与其他 / Organic & Other / Organic & Other |
| tree 源 | `data/trees/cambridge/igcse/chemistry.json` |

## 课标原文（Cambridge Syllabus）

### 课标原文

describe the structure of a DNA molecule as a double helix, including:
• the importance of complementary base pairing between
the 5′ to 3′ strand and the 3′ to 5′ strand (antiparallel
strands)
• differences in hydrogen bonding between C–G and A–T
base pairs
• linking of nucleotides by phosphodiester bonds

describe the semi-conservative replication of DNA during the
S phase of the cell cycle, including:
• the roles of DNA polymerase and DNA ligase (knowledge
of other enzymes in DNA replication in cells and different
types of DNA polymerase is not expected)
• the differences between leading strand and lagging strand
replication as a consequence of DNA polymerase adding
nucleotides only in a 5′ to 3′ direction

## 知识图谱关系

### 前驱知识

- `chemistry-cam-igcse-redox`

## 课标摘要

describe the structure of a DNA molecule as a double helix, including: • the importance of complementary base pairing between the 5′ to 3′ strand and the 3′ to 5′ strand (antiparallel strands) • differences in hydrogen bonding between C–G and A–T base pairs • linking of nucleotides by phosphodiester bonds；describe the semi-conservative replication of DNA during the S phase of the cell cycle, including: • the roles of DNA polymerase and DNA ligase (knowledge of other enzymes in DNA replication in cells and different types of DNA polymerase is not expected) • the differences between leading strand and lagging strand replication as a consequence of DNA polymerase adding nucleotides only in a 5′ to 3′ direction
