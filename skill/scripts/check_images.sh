...
if [ "$img_files" -ge 3 ] && [ "$img_refs" -ge 3 ]; then
  ...
  echo ""
  echo "建议立即用 image_gen 生成 3 张图（命名规范见 docs/IMAGE-NAMING-SPEC.md）：" 
  echo "  1) Hero 氛围图 → assets/<abbrev>-hero.png"
  echo "  2) 核心概念可视化 → assets/<abbrev>-scene.png"
  echo "  3) 拓展插图 → assets/<abbrev>-ext.png"
  echo "  （<abbrev> = 课件ID核心词，如 linear, quadratic, pressure 等）"
  echo ""
