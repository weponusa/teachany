...
#
# 用法：
#   bash build_chgis_dynasty_maps_v2.sh <teachany-opensource 仓库根目录>
# 或设置环境变量：
#   TEACHANY_REPO=/path/to/teachany bash build_chgis_dynasty_maps_v2.sh

set -e

CACHE="/tmp/chgis_v6/cache.json"

# 动态定位输出目录（不再硬编码 /Users/wepon/...）
REPO="${1:-${TEACHANY_REPO:-}}"
if [ -z "$REPO" ]; then
...
if [ -z "$REPO" ] || [ ! -d "$REPO" ]; then
  echo "❌ 未找到 teachany-opensource，请传入路径或设置 TEACHANY_REPO 环境变量"
  echo "   用法：bash build_chgis_dynasty_maps_v2.sh /path/to/teachany-opensource"
  exit 1
fi
OUT_DIR="$REPO/data/_legacy/resources/geography/historical-china"
mkdir -p "$OUT_DIR"
echo "输出目录: $OUT_DIR"
