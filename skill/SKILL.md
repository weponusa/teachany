...
| ⑤ | Hero knowledge-structure infographic (top of page). If image generation is available, prefer a high-quality AI-generated hero base plus controlled HTML/SVG/Pillow text overlays; do not default to low-quality Pillow-only covers. | Hero 图 / 知识结构图 / 高质量生图 | Gives students the mental map before details while preserving visual trust |
...
- **Image sourcing decision tree for items ④⑤⑯**: (a) Check `assets/image-registry.json` / `assets/maps/MANIFEST.json` / CDN first; (b) if no match and image is a **specific historical artwork** (e.g., Mona Lisa, David sculpture), search web for a public-domain scan — Wikimedia Commons is the canonical source; (c) if no match and image is a **Hero / educational concept illustration**, use high-quality `image_gen` with a subject-specific prompt; (d) for Hero text, prefer controlled HTML/SVG/Pillow overlays instead of asking image models to render dense Chinese text; (e) never fake "see this image" without an actual image.
