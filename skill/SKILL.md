...
9. **依赖分级**：Python/Git/浏览器验证是核心依赖；Remotion、edge-tts、ffmpeg、image_gen 是增强依赖。快速模式允许增强依赖降级，完整发布必须补齐或明确用户豁免。
...
当前执行摘要版本：`7.13.0`。历史变更不放入主文件，避免污染执行上下文；需要考古时查 Git 历史或仓库发布记录。
