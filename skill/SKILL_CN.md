...
| **② Remotion 程序化动画** | 课件必须含 **≥1 段 Remotion 生成的教学动画**（演示过程性变化） | Remotion + React + TS 渲染 1920×1080 @30fps → 输出 `assets/*.mp4` → 嵌入对应 section | 至少用 Canvas/SVG 做等价交互动画替代，不得只放静态图 |
...
1. **Phase 0（需求确认）末尾**：必须输出"基线能力开启清单"，明确声明 TTS/Remotion/Canvas/生图 四项全开（除非用户拒绝）
2. **Generation Gate**：基线四项任一标注"跳过"必须附理由，且理由会被 Completeness Gate 二次审查
3. **Phase 3（制作）**：若环境支持 `task` 工具，必须并行分发 Agent C（生图）+ Agent D（TTS）+ Remotion 渲染任务
4. **Completeness Gate**：四项全部校验，缺一不通过
...
## 十三、34 条硬规则（违反任何一条 = Completeness Gate 不通过）
...
| 32 | **基线强制 · Remotion 教学动画**（见 Section 0）。每个课件必须含 ≥1 段 Remotion 程序化动画用于演示过程性变化（含文科的意境动画、诵读节奏动画、历史演变动画等）。若用 Canvas/SVG 交互替代，必须在 Gate 中列出等效性说明。 | 无 Remotion 且无等效替代 → Gate 直接不通过 |
