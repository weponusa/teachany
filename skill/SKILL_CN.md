...
**3.2 L2 教学动画（默认必选，自动执行）**：

> ⚠️ **L2 是默认必选项**。L1 课件生成完毕后，AI 必须立即自动执行 L2 教学动画生成流程，不需要等待用户确认。
> 仅当用户在最初下达任务时明确说了"不要动画/不要视频/不要Remotion"时，才跳过此步骤。
>
...
> ✅ **通过条件**：L1 课件已完成（含双语版本）；L2 教学动画已生成（或用户明确拒绝，或环境不可用且用户确认放弃）；L3 语音已生成（或用户明确拒绝，或环境不可用且用户确认放弃）；课件已打包（Phase 3.5）。
> ❌ **阻断**：课件中没有互动练习 = 不通过；没有前测或后测 = 不通过；L2 未执行且未获得用户明确跳过确认 = 不通过；L3 未执行且未获得用户明确跳过确认 = 不通过；未生成双语版本且用户未明确拒绝 = 不通过。
...
□ 14. AI 互动区（默认检查，仅 Gate 标注"跳过"时标 N/A）：是否在指定位置插入了多模态互动区？
     → ✅/❌/N/A ___
□ 15. 双语课件：是否同时生成了中文 index.html 和英文 index_en.html？
     → ✅/❌/N/A（用户拒绝） ___
□ 16. 课件打包：是否生成了 manifest.json 并完成 .teachany 打包？
     → ✅/❌ ___
□ 17. 知识图谱溯源：课件核心内容是否可追溯到知识图谱/Web搜索/明确标注的来源？
     → ✅/❌ ___
□ 18. L2 教学动画（默认检查）：是否已生成 Remotion 教学动画或保留完整源码+降级说明？
     → ✅/❌/N/A（用户拒绝 或 环境不可用且用户确认放弃） ___

【总评】___ / 18 通过（#14 仅纯计算/纯习题/纯复习课可标 N/A；#15 用户拒绝时可标 N/A；#18 用户拒绝或环境不可用且用户确认放弃时可标 N/A）
【修复动作】（如有未通过项）___
═══════════════════════════════════════════
```

> ✅ **通过条件**：必检项（#1-#18）全部 ✅ 或 N/A（#14 仅纯计算/纯习题/纯复习课可 N/A；#15 用户拒绝时可 N/A；#18 用户拒绝或环境不可用且用户确认放弃时可 N/A），或未通过项已给出修复方案并执行修复。
> ❌ **阻断**：有 3 项以上 ❌ = 必须修复后再交付。

### Phase 4：交付与输出决策 ⛔ CHECKPOINT

1. **交付 L1 课件**（必选）：
   - 生成完整的 `index.html`（中文）和 `index_en.html`（英文）文件
   - 确认所有互动功能可运行

2. **L2 教学动画确认**：
   - L2 教学动画已在 Phase 3.2 自动生成 → 在文件清单中列出生成的视频文件
   - 如用户明确拒绝了 L2 → 标注"用户要求跳过教学动画"
...
> ✅ **通过条件**：课件已交付（含双语）；L2 已完成（或用户已拒绝，或环境不可用且用户确认放弃）；L3 已完成（或用户已拒绝，或环境不可用且用户确认放弃）；课件已打包；文件清单已列出。
...
| **L2** | 教学动画 | ✅ **默认必选** | 自动执行，除非用户明确拒绝 |
| **L3** | AI 语音讲解 | ✅ **默认必选** | 自动执行，除非用户明确拒绝 |

### L1 — 互动课件（必选，不可跳过）

- `public/index.html`：完整互动网页课件（中文）
- `public/index_en.html`：完整互动网页课件（英文）— **默认生成**，除非用户明确说"只要中文"
...
> ⚠️ **L2 与 L1 同为必选层级**。每个课件生成后，AI 必须自动完成环境搭建并生成教学动画视频，无需等待用户确认。
> **唯一跳过条件**：用户在下达任务时明确说了"不要动画""不要视频""不要Remotion""不需要动画"等拒绝性表述。

**自动执行流程**（L1 课件完成后立即执行）：
1. 检测 Node.js ≥ 18 → 缺失则自动安装
2. 检测 ffmpeg → 缺失则自动安装
3. 生成项目配置（package.json、tsconfig.json、remotion.config.ts）
4. 执行 `npm install`
5. 编写 Remotion 动画组件（`src/compositions/*.tsx`）
6. 生成音效（`node generate-sfx.js`）
7. 渲染视频（`npm run build:all`）

**降级策略**：
| 情况 | 处理方式 |
|:---|:---|
| npm install 失败（网络问题） | 保留全部 Remotion 源码和配置，提示用户联网后执行 `npm install && npm run build:all` |
| ffmpeg 安装失败 | 保留源码，提示用户安装 ffmpeg 后执行 `npm run build:all` |
| 渲染超时或内存不足 | 保留源码，降低分辨率至 1280×720 重试；仍失败则提示用户在本地渲染 |
| Node.js 不可用且无法安装 | 生成全部 Remotion 源码 + package.json，提示用户安装 Node.js 后执行 |

> ⚠️ **放弃选项**：如果上述任何降级步骤仍无法解决，AI 必须明确给用户两个选择：
>
> 1. **保留源码，稍后手动构建**：所有 Remotion 代码和配置已就绪，用户解决环境问题后可随时执行构建命令
> 2. **放弃 L2，仅交付 L1+L3+L4**：清理 Remotion 相关文件，课件以互动网页+语音讲解形式交付，Completeness Gate #18 标注 N/A（环境不可用）
>
> AI 不可替用户做决定，必须等待用户明确选择后再继续。
...
> ⚠️ **放弃选项**：如果上述降级步骤仍无法解决，AI 必须给用户两个选择：
>
> 1. **保留脚本，稍后手动生成语音**：旁白 JSON 和 TTS 脚本已就绪，用户解决环境后执行即可
> 2. **放弃 L3，仅交付 L1+L2+L4**（或 L1+L4）：课件以纯互动网页形式交付，Completeness Gate 中 L3 相关项标注 N/A（环境不可用）
...
L2 和 L3 均已默认执行，无需建议。如果 L2 执行过程中遇到环境问题（Node.js 不可用、npm install 失败、ffmpeg 缺失、渲染超时等），必须：
1. 保留所有已生成的 Remotion 源码和配置文件
2. 在交付时明确告知用户哪个步骤失败、具体错误信息
3. 给出用户可执行的修复命令（如 `npm install && npm run build:all`）
4. **不可因为环境问题而直接跳过 L2，也不可悄悄删除已生成的源码**
5. **如果自动修复和降级均失败，必须给用户明确的放弃选项**：

> 📋 **环境问题处理模板**（AI 必须按此格式向用户提示）：
>
> ```
> ⚠️ L2 教学动画环境搭建遇到问题：[具体错误描述]
>
> 已尝试的修复：[列出已尝试的步骤]
>
> 请选择：
> A. 保留源码，我稍后手动修复环境并构建
>    → 执行命令：[具体修复+构建命令]
> B. 放弃本次 L2 教学动画，先交付课件+语音
>    → 课件以 L1+L3+L4 形式交付，不影响使用
> ```
>
> 用户选择 B 后，Completeness Gate #18 标注 **N/A（环境不可用，用户确认放弃）**，Phase 3 通过条件视为满足。

---

## 十三、20 条硬规则（违反任何一条 = Completeness Gate 不通过）
...
| 19 | **双语课件默认生成**。除非用户明确说"只要中文"或"只要英文"，否则同时生成 `index.html`（中文）和 `index_en.html`（英文）。 | 缺少双语版本 → 国际化体验不完整 |
| 20 | **L2 教学动画默认必选**。L1 课件完成后必须自动搭建 Remotion 环境并生成教学动画视频。仅用户明确拒绝时跳过。不可因为"环境复杂""节省 token""节省时间"等理由擅自跳过。环境安装失败时必须保留全部源码并给出修复命令；如降级措施均失败，必须给用户明确的 A/B 选项（保留源码 vs 放弃 L2），不可替用户做决定。 | 无教学动画且未获用户确认 → 课件演示能力不完整 → Phase 3 不通过 |
...
|:-----|:-----|:-----|:------|:---------|
| **L1 — 互动课件** | `public/index.html` + `public/index_en.html` | 无（零依赖） | 不需要 | ✅ 始终提供（默认双语） |
| **L2 — 教学动画** | `out/*.mp4`（Remotion 渲染） | Node.js ≥ 18、npm、ffmpeg | ✅ AI 自动检测安装 | ✅ **默认必选** |
...
**L1 + L2 + L3 + L4 始终自动执行。用户可通过明确拒绝来跳过 L2 或 L3。**

### 15.2 L2 环境自动搭建（Remotion）

> **L2 为默认必选。L1 课件完成后，AI 必须自动执行以下全部步骤，不等待用户确认。仅用户明确拒绝时跳过。**
...
| **以上均失败** | **给用户 A/B 选项：A. 保留源码稍后手动构建；B. 放弃 L2，先交付 L1+L3+L4** |
...
| **以上均失败** | **给用户 A/B 选项：A. 保留脚本稍后手动生成语音；B. 放弃 L3，先交付 L1+L2+L4** |
...
- "做一个中文课件，不要动画" → L1+L3+L4，跳过 L2
- "做一个中文课件，不要动画也不要语音" → 仅 L1+L4，跳过 L2/L3
...
|:-----|:----------|:-----------|:---------|
| **仅 L1**（中文 HTML，用户跳过 L2+L3） | ~20K | ~35K | ~55K |
| **L1 双语**（中英 HTML，用户跳过 L2+L3） | ~25K | ~70K | ~95K |
| **L1+L2**（HTML + Remotion，用户跳过 L3） | ~30K | ~100K | ~130K |
| **L1+L2+L3 全量** ⭐ **默认**（HTML + 视频 + TTS + 字幕，双语） | ~35K | ~120K | ~155K |

> 注：v5.8 起 L1+L2+L3+L4 四层全部默认执行，因此默认场景的 Token 消耗为 **~155K**。上表中"仅 L1"等场景仅在用户明确拒绝 L2/L3 时出现。

### 16.3 按模型估算成本

| 模型 | 输入单价 | 输出单价 | 仅 L1（用户跳过 L2+L3） | L1+L2+L3 全量 ⭐ 默认 |
|:-----|:---------|:---------|:------|:---------------|
