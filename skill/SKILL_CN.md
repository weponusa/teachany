...
| **② Remotion 程序化动画** | 课件必须含 **≥1 段真正用 Remotion 渲染的教学动画 mp4**（演示过程性变化） | Remotion + React + TS 渲染 1920×1080 @30fps → 输出 `assets/*.mp4` → 嵌入对应 section | ⛔ **无降级**。Canvas/SVG/CSS 动画不得替代 Remotion 基线。缺 Remotion = 直接 Gate 不通过。特殊情况（无 Node 环境且无法安装）必须在 Gate 中明确声明并由用户书面豁免 |
...
5. **Completeness Gate**：四项全部校验（Remotion 必须检查 `assets/*.mp4` 真实存在且已嵌入 HTML），缺一不通过
