...
**音频交互补充**：连续音频不应作为一个集中“音频模块”插在学习路径中。`data-teachany-audio-playlist` 只作为隐藏配置存在，学生通过底部连续播放条、section 滚动同步和必要的局部播放入口使用音频，避免打断教学主线。
...
**生图优先原则（v7.12.3）**：如果当前环境有 `image_gen` / Nano Banana / Gemini Image 等生图能力，Hero 图优先使用高质量 AI 生图做主视觉，不再默认用 Pillow 低质绘图。正确做法是“AI 高质量底图 + 可控文字层”：生图负责场景质感、光影、构图和学科隐喻；中文标题、公式、模块节点、箭头关系用 HTML/SVG/Pillow `font_resolver.py` 叠加，避免 AI 生图文字乱码。只有生图不可用或连续失败 ≥3 次，才允许使用 SVG/Pillow 兜底。
...
| 4 | **文字可读** | 图上所有文字 ≥ 18px 可读；不可出现 Pillow 生图产生的 ⊠ .notdef 方框；本地 Pillow 生图必须先运行 `python3 skill/scripts/font_resolver.py --check --print-path`，并用 `get_pillow_font()` 加载字体，严禁 `ImageFont.load_default()` 兜底 |
