...
| 39 | **地图资源基线 · Leaflet hillshade/GeoJSON 必须用"本地相对路径 + jsDelivr CDN 回退"双路径**（v5.21 新增，见 Section 18.5.2）。GitHub Pages 对 `data/geography/` 下的 `.jpg` / 大型 `.geojson` 存在**跳过部署**现象（即便文件已 tracked 并 push 到 main，raw.githubusercontent.com 能 200，但 `<user>.github.io/<repo>/data/geography/xxx.jpg` 返回 404）。必须使用本 SKILL 提供的 `addSmartImageOverlay()` / `geoFetchJson()` 工具函数：先尝试 `../../data/geography/...` 本地路径（本地预览 + 其他 host 场景），失败则自动回退到 `https://cdn.jsdelivr.net/gh/<user>/<repo>@main/data/geography/...`。⛔ 严禁直接 `L.imageOverlay('../../data/...')` 硬编码单一路径。发布前必须用 `curl -I https://<user>.github.io/<repo>/data/geography/hillshade/*.jpg` 验证线上 200，返回 404 就**不得声称"地图底图已上线"**。 | 底图/GeoJSON 线上 404 / 硬编码单路径 → Gate 直接不通过 |
