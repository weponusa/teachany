...
| 48 | **发布权分离基线 · 默认路径是 `community/drafts/`，AI 必须先询问用户再决定是否提交**（v5.34.8 新增，修复"用户直接上传成官方课件"严重漏洞）。**根因**：v5.34.8 之前的 SKILL Phase 3.5 Step 3 写着"质检通过 = 直接上架，不需要管理员审核"，Step 4 又用"工作区是否有 scripts/rebuild-index.py"来判断"管理员模式"——结果任何克隆了 teachany-opensource 仓库的普通用户在本地生成课件后，AI 都会**自动**把课件写入 `examples/`（官方目录）、修改 `registry.json`、修改 `data/trees/*.json`、然后 `git push origin main && git push gitee main`，直接污染官方 Gallery。实测数据：2026-04-19 盘点 `registry.json` 时，138 份课件里 124 份 status=community 实际上是"用户本地 AI 自动推上来的"，根本没经过任何人审核。**双轨制设计（v5.34.8）**：AI 生成课件后**一律先写到 `community/drafts/<course-id>/`**（被 `.gitignore`），然后**强制询问**用户意向（① 仅本地 / ② 提交社区 / ③ 管理员直推），按用户明确回复执行下一步：用户选 ② → 调用 `scripts/submit-to-community.py <course-id>`，脚本通过 `repository_dispatch` 事件触发 `community-submit.yml` workflow，GitHub Actions 自动创建分支 + 开 PR 到 `community/pending/`，后续走正规 PR 审批流程（`approved` / `promote-to-official` / `revision-needed`）；用户选 ③ → 三重门（`.teachany-admin` 标记 + 明确发布关键词 + 用户复核）全部成立才允许 `mv` 到 `examples/` 并手工改 `registry.json` 的 status=official + rebuild-index + git push。**AI 绝对禁止的自作主张**：(a) 未询问用户就直接 `git push`；(b) 未经用户明确选择就把课件写入 `examples/` 或改 `registry.json`；(c) 仅凭"工作区叫 teachany-opensource"或"存在 scripts/rebuild-index.py"就跳过三重门；(d) 用"git push 更方便"作为绕开 `submit-to-community.py` 的理由（脚本的审批链是社区治理的核心，不能为了省事绕开）；(e) 在新增课件时把 `registry.json` 的 status 打成 `official`（新课件一律默认 `community`，升格必须管理员手工编辑）。**`rebuild-index.py` 硬门槛**：脚本首行检查 `.teachany-admin` 不存在就 `exit 2`，并向用户打印三条明确提示（这是普通用户 vs 管理员的分水岭）。**`submit-to-community.py` 脚本**：读取 `.teachany-token`（Fine-grained token 最小权限 Contents/Metadata Read-only），打包 `.teachany` → `repository_dispatch` → GitHub Actions 自动开 PR；首次使用时引导用户一次性配置 token，配置一次终身有效。**"质检通过"不等于"发布成功"**：质检只保证课件本身合格，不代表可以进入官方索引；质检通过 → 写 drafts → 询问用户 → 按用户意愿执行。**双向保护**：本条既保护了仓库 owner（防止 AI 误污染官方 Gallery），也保护了普通用户（防止 AI 未经同意就公开可能含隐私/版权敏感的课件内容）。 | AI 未询问用户就 `git push` / 默认把课件写入 `examples/` / 新课件 registry 自动 `status=official` / 跳过 `submit-to-community.py` 直接 push / 克隆仓库后 `rebuild-index.py` 未校验 `.teachany-admin` 就运行 → Gate 直接不通过（这是最严厉的一条，发布失败不是警告是 exit 1） |
