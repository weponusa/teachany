...
## 十三、47 条硬规则（违反任何一条 = Completeness Gate 不通过）
...
| 48 | **发布权分离基线 · 默认路径是 `community/drafts/`，绝对禁止 AI 默认 git push**（v5.34.8 新增，修复"用户直接上传成官方课件"严重漏洞）。**根因**：v5.34.8 之前的 SKILL Phase 3.5 Step 3 写着"质检通过 = 直接上架，不需要管理员审核"，Step 4 又用"工作区是否有 scripts/rebuild-index.py"来判断"管理员模式"——结果任何克隆了 teachany-opensource 仓库的普通用户在本地生成课件后，AI 都会**自动**把课件写入 `examples/`（官方目录）、修改 `registry.json`、修改 `data/trees/*.json`、然后 `git push origin main && git push gitee main`，直接污染官方 Gallery。实测数据：2026-04-19 盘点 `registry.json` 时，138 份课件里 124 份 status=community 实际上是"用户本地 AI 自动推上来的"，根本没经过任何人审核。**三重门管理员识别（缺一不可）**：① 工作区根目录存在 `.teachany-admin` 标记文件（owner 本地手工 `touch` 创建，已被 `.gitignore` 拦住）；② 用户对话中出现明确发布关键词（"发布到官方"/"promote to official"/"直推 origin"）；③ AI 已单独向用户复核"课件去向"并等到明确回复"examples/"。**默认行为**：AI 生成的课件**一律**先写到 `community/drafts/<course-id>/`（被 `.gitignore`），交付后仅告知用户三条后续路径（本地用 / PR 贡献 / 管理员手工升格），**绝不自动** `git add`/`commit`/`push`。**`rebuild-index.py` 硬门槛**：脚本首行检查 `.teachany-admin` 不存在就 `exit 2`，防止普通用户误跑；新增课件默认 `status=community`，升格 `official` 必须管理员手工编辑 `registry.json`。**"质检通过"不等于"发布成功"**：质检只保证课件本身合格，不代表可以进入官方索引；质检通过 → 写 drafts → 告知用户 → 停在这里。 | AI 未经用户明确发布指令就 `git push` / 默认把课件写入 `examples/` / 新课件 registry 里自动打成 `status=official` / 克隆仓库后 `rebuild-index.py` 未校验 `.teachany-admin` 就运行 → Gate 直接不通过（这是最严厉的一条，发布失败不是警告是 exit 1） |
...
Step 3️⃣ 课件落地到本地工作区（默认行为）
  课件做完+打包+质检通过后，**仅在用户本地工作区落地**，不自动发布、不自动推送：
  
  1. 将课件文件保存到 `community/drafts/<course-id>/`（index.html + manifest.json + assets）
     - ⛔ **禁止默认写入 `examples/`**：`examples/` 是官方策划课件目录，只能由仓库管理员 cherry-pick 审核后放入
     - ⛔ **禁止默认修改 `registry.json` / `courseware-registry.json` / `data/trees/*.json`**：这些都是官方索引文件，不是用户本地文件
  2. 在 `community/drafts/index.json` 中追加条目，`status` 设为 `"draft"`（仅供本地 Gallery 预览用，不影响官方索引）
  3. 告知用户课件已完成，并给出三条明确后续选项：
     - ① 本地预览：直接用浏览器打开 `community/drafts/<course-id>/index.html`
     - ② 贡献到社区：按 `community/README.md` 的 PR 审批流程（路径 A/B/C），由用户**主动**提交 PR 到 `community/pending/`
     - ③ 仅自用：什么都不做，课件文件留在本地
  
  > ⛔ **AI 绝对不能自动执行 `git push`**（除非满足 Step 4 的管理员模式全部条件）。**"质检通过 ≠ 直接上架"** —— 这是 v5.34.8 纠正的核心漏洞：早期版本里"默认自动执行 git push"会把任何用户生成的课件一路推到官方 Gallery，严重污染了 `examples/` 与 `registry.json`。自 v5.34.8 起，发布权从 AI 手里彻底收回到人工 PR 审批流程。

Step 4️⃣ 管理员直推（⚠️ 仅当三重条件全部成立才执行，v5.34.8 收紧）
  
  AI 必须**依次**检查以下三重条件，任何一条不成立都必须退回到 Step 3：
  
  条件 A（仓库标记）：工作区根目录必须存在一个显式的管理员标记文件 `.teachany-admin`
     ```bash
     test -f .teachany-admin || { echo "非管理员工作区，退回 Step 3"; exit 0; }
     ```
     - ⛔ 仅靠"工作区名叫 teachany-opensource"或"存在 scripts/rebuild-index.py"**不构成管理员身份**（v5.34.8 纠正：旧规则被克隆仓库的普通用户误触发）
     - `.teachany-admin` 文件由仓库 owner 在本地手工创建，且**不会**被 git 跟踪（已在 .gitignore 中）
  
  条件 B（用户显式指令）：当前对话中用户必须包含**明确的发布意图关键词**之一：
     - "发布到官方"、"提升为官方"、"promote to official"、"合并到 examples"、"直推 origin"
     - ⛔ "做好就行"、"课件做好了吗"、"完成了就推一下吧"等含糊指令**不满足**本条件
     - ⛔ Phase 0 用户填写的"是否双语/是否 pptx"等默认选项**不包含**发布意图
  
  条件 C（课件定位复核）：AI 必须向用户**单独复核一次**要把课件放在哪里，并等待用户回复明确选项：
     ```
     请确认此课件的最终去向（回复数字）：
     ① community/drafts/  — 仅本地，不推送（默认）
     ② community/pending/ — 提 PR 到社区仓，等管理员审核
     ③ examples/         — 官方直推（需管理员身份 + 承担上架责任）
     ```
     - ⛔ 未收到用户明确回复就自动走 ③ 属于 v5.34.8 之前的漏洞
  
  三重条件全部成立后，才能执行：
  1. 将课件从 `community/drafts/<course-id>/` 移动或复制到 `examples/<course-id>/`
  2. 手工修改 `registry.json` 中该课件的 `status` 为 `official`（默认仍是 community）
  3. 重建索引和知识树：
     ```bash
  ...
  4. 提交并双推：
     ```bash
     git add -A && git commit -m "feat: 新增官方课件 <course-id>" && git push origin main && git push gitee main
     ```
  5. 输出课件在线地址：
     `https://weponusa.github.io/teachany/examples/<course-id>/`
  
  ⚠️ 三重条件任何一条不满足 → 静默退回 Step 3，绝不能半自动执行（比如只跑前两步 rebuild-index）。
```
...
3. **课件落地 + 身份识别**（v5.34.8 收紧）：
   
   **默认：本地草稿模式**（对所有用户都是默认行为）：
   ```bash
   # 课件只写到 community/drafts/<course-id>/，不改官方索引、不推送
   mkdir -p community/drafts/<course-id>
   cp -r <生成目录>/* community/drafts/<course-id>/
   # 可选：追加到 community/drafts/index.json（仅供本地 Gallery 预览）
   ```
   交付后告知用户三条后续路径：① 仅本地用 ② 走 PR 贡献到社区 ③ 如你是仓库管理员，请另行手工 cherry-pick 到 examples/
   
   **管理员模式触发条件**（v5.34.8 三重门，缺一不可）：
   - 条件 A：工作区根目录存在 `.teachany-admin` 标记文件（owner 本地手工创建，`.gitignore` 不跟踪）
   - 条件 B：用户对话中出现明确发布关键词（"发布到官方"/"promote to official"/"直推 origin"）
   - 条件 C：AI 已单独向用户复核"课件去向"并收到明确选择 examples/ 的回复
...
- v5.34.8：**⭐ 发布权分离 · 修复"用户直接上传成官方课件"严重漏洞 · 新增硬规则 #48**——(1) 用户反馈"而且怎么用户直接上传成官方课件了？"。(2) **根因诊断**：v5.34.8 之前的 SKILL Phase 3.5 存在双重漏洞——Step 3 直白写着"质检通过 = 直接上架，不需要管理员审核"，Step 4 用"工作区是否存在 `scripts/rebuild-index.py`"来判断"管理员模式"。结果任何克隆了 teachany-opensource 仓库的用户，在本地生成课件后 AI 都会**自动**把课件写入 `examples/`（官方目录）、修改 `registry.json`/`data/trees/*.json`、然后 `git push origin main && git push gitee main`。盘点 2026-04-19 数据：`registry.json` 中 138 份课件里 **124 份 status=community 实际上是用户本地 AI 自动推上来的**，完全跳过了 `community/README.md` 里明文规定的 PR 审批流程（路径 A/B/C）。(3) **SKILL 层修复**：Phase 3.5 Step 3 重写为"课件一律先落地到 `community/drafts/<course-id>/`，不自动推送，交付后仅告知用户三条后续路径"；Step 4 管理员直推改为**三重门**——必须同时满足 ① 根目录存在 `.teachany-admin` 标记文件；② 用户对话中出现明确发布关键词（"发布到官方"/"promote to official"/"直推 origin"）；③ AI 已单独向用户复核课件去向并收到明确回复。三条任何一条不满足 → 静默退回草稿模式。Section 17.4 同步重写，删掉"自动判断身份并发布"的诱导逻辑。(4) **`rebuild-index.py` 硬门槛**：脚本启动首先检查 `.teachany-admin` 不存在就 `exit 2`，并向用户打印三条明确提示（这是普通用户 vs 管理员的分水岭）。新增课件默认 `status=community`（而非根据位置自动提升为 official），升格必须管理员手工编辑 `registry.json`。(5) **新增 `community/drafts/` 目录**：AI 默认落点，带 README 解释设计意图和三条后续路径；`.gitignore` 把 `community/drafts/` 整体忽略但保留 `README.md` 和 `.gitkeep`，这样用户克隆仓库时能看到说明、本地生成的课件又不会进入 git。(6) **新增硬规则 #48 发布权分离基线**：明确"AI 默认不自动 git push"、"默认路径是 `community/drafts/`"、"`status=official` 必须管理员手工设置"、"`.teachany-admin` 三重门"；硬规则从 47 条扩到 **48 条**。(7) **设计哲学**：把"发布权"从 AI 手里彻底收回到人工 PR 审批流程。"质检通过"不等于"发布成功"——质检只保证课件本身合格，不代表可以进入官方索引。AI 的职责是做出好课件 + 写到 drafts + 告知用户，绝不能自作主张推到官方仓。
