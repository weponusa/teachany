...
## 十三、44 条硬规则（违反任何一条 = Completeness Gate 不通过）
...
| 44 | **节点唯一挂载基线 · 同一 `node_id` 严禁被多份课件同时挂载**（v5.29 新增）。`data/trees/*.json` 的每个节点代表一个课标知识点，**最多只能绑定 1 份课件**（`status=official` 优先于 `community`）；Gallery 按 `node_id` 聚合展示时，若同一节点下出现多份不同 `course_id`、不同路径、不同内容的 HTML，用户会在同一知识点卡片上看到互相冲突的两个版本（如曾出现 `phy-mid-pressure` 与 `teachany-phy-mid-pressure` 都挂 `phy-m-pressure`、一份 community 单页简版、一份 official 深色完整版）。⛔ 严禁在 examples/ 下同时保留"内容相同 / 节点相同 / 仅目录名不同"的课件目录——这通常是课件重命名或 `teachany-` 前缀历史遗留导致。**发布前强制校验**：`scripts/validate-courseware.py` 末尾的跨课件扫描阶段会按 `manifest.node_id` 聚合所有课件，若发现 `len(items) > 1` 立即报错并列出全部冲突条目 `id(status)` ，退出码 1 阻断发布。**冲突解决规则**：(a) 两份内容完全相同 → 保留目录名更规范（不带 `teachany-` 冗余前缀）且 `tts/` 时间戳更新的一份；(b) 两份内容不同但 node 相同 → 保留 `status=official` 那份，把另一份要么改挂到其他合适节点、要么删除；(c) 其中一份仅是孤儿音频/源文件包（无 `index.html` 或 `manifest.json`） → 直接删除。⛔ 严禁仅靠 `course_id` 不同就认为它们是两个独立课件——节点才是 Gallery 聚合键。 | 同一 node_id 挂 ≥2 份课件 / 删课件后未 rebuild-index / 未跑 validate-courseware 就 push → Gate 直接不通过 |
