...
- 纯 Canvas 课件：用 `drawImage()` 绘制为底图层
- Leaflet 课件：用 `L.imageOverlay()` 铺底
- ECharts 课件：用 `graphic` 组件铺底
...
Promise.all([
  ...
  chart.setOption({
    ...
    geo: { map: 'qin', roam: true, itemStyle: { areaColor: '#3CB371' } },
    series: [{
      type: 'scatter',
      coordinateSystem: 'geo',
      data: qinData.landmarks.map(lm => ({
        name: lm.name, value: [lm.lng, lm.lat]
      })),
      symbolSize: 10,
