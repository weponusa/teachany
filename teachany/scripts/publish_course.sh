...
# ─── 1.2. node_id 预校验（课标节点或 ext-* PBL 外部知识点）─────
if [ -f "$SKILL_SCRIPTS/check_node_id.py" ] && [ -f "$SRC_DIR/index.html" ]; then
  ...
  if ! python3 "$SKILL_SCRIPTS/check_node_id.py" "$SRC_DIR" > /tmp/node_check.log 2>&1; then
    ...
    echo ""
    echo "  💡 处理方式："
    echo "     A) 课标内课件：修正为已有 node_id（find_nodes.py）"
    echo "     B) PBL 课标外：使用 ext-{8位hex}，与 teachany-node / manifest 一致"
    echo "     C) 勿再用 teachany-free-mode 代替挂树（已废弃用于「其他知识」）"
    exit 1
  else
    grep -E "✅|节点名|其他知识|ext-" /tmp/node_check.log | head -4
  fi
fi
...
# free_mode：仅当非 ext-* 时清空 node_id（ext-* 必须保留以进「其他知识」）
if free_mode:
    if node_id and re.match(r"^ext-[a-f0-9]{6,12}$", node_id, re.I):
        print(f"  💡 free_mode=true 但保留 PBL ext 节点: {node_id}")
    else:
        if node_id:
            print(f"  ⚠️  free_mode=true，丢弃 node_id='{node_id}'（无法挂「其他知识」，仅社区展示）")
        node_id = ""
