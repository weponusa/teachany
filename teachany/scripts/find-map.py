...
# 可用环境变量 TEACHANY_MAP_REMOTE_BASE 覆盖首选源。
REMOTE_MAP_BASES = [
...
def load_manifest():
    ...
    for base in REMOTE_MAP_BASES:
        ...
        try:
            ...
            data["_source"] = url
            data["_remote"] = True
            data["_remote_base"] = base
            return data
        except Exception as e:
            errors.append(f"{base}: {e}")
            continue
    sys.exit("❌ 找不到本地地图 manifest，双平台远端均加载失败：\n  " + "\n  ".join(errors))
...
def copy_resource(filename, dst_dir):
    ...
    if local_candidates:
        ...
        # 双平台依次尝试：teachany.cn → jsDelivr → raw.github
        errors = []
        source_label = None
        for base in REMOTE_MAP_BASES:
            url = base + "/" + rel
