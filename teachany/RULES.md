...
- **#21a** **现代中国地图标准做法：自带晕渲底图+省界（v2.9 起）**：地理课件做现代中国地图时，用 `"crs":"EPSG4326"` + `"baseImage":{"url":"../../assets/maps/physical/hillshade/global-color-hillshade-4k.jpg","bounds":[[-90,-180],[90,180]]}`（自带 Natural Earth 彩色晕渲，离线/国内可访问；EPSG:4326 下等距圆柱 JPG 与 WGS84 GeoJSON 经纬度线性对齐，**不违反 #21**——#21 禁止的是 Web Mercator 下错位的 imageOverlay）；省界用 `../../assets/maps/political/china-modern/provinces.geojson`（35 省级要素），era 设 `fillOpacity:0.05~0.12` 让地形透出；课件专属要素（铁路/路线等）写本地 GeoJSON 用 `overlays` 叠加。备选：`basemap` 配自带瓦片 `physical/terrain-tiles/{z}/{x}/{y}.png`（zoom4-6）时必须加 `minNativeZoom:4,maxNativeZoom:6`（否则地图 zoom<4 底图空白）。参考实现：`community/geo-m-china-transportation/`。
