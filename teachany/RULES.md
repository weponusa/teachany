...
- **#21a** **现代中国/世界地图标准做法：自带晕渲底图+EPSG4326（v2.9 起）**：用 `"crs":"EPSG4326"` + `"baseImage":{"url":"../../assets/maps/physical/hillshade/global-color-hillshade-4k.jpg","bounds":[[-90,-180],[90,180]]}`（自带 Natural Earth 彩色晕渲，离线/国内可访问；EPSG:4326 下等距圆柱 JPG 与 WGS84 GeoJSON 经纬度线性对齐，**不违反 #21**——#21 禁止的是 Web Mercator 下错位的 imageOverlay）；省界用 `political/china-modern/provinces.geojson`（35 省级），世界政区用 `political/world/countries.geojson`，era 设 `fillOpacity:0.05~0.12` 让地形透出；课件专属要素（铁路/路线等）写本地 GeoJSON 用 `overlays` 叠加。**v2.9 配置项**：`refitEra:false`（全球轮廓类 era 会抢焦点时用，强制尊重 `fitBounds` 教学聚焦）；`zoomSnap:0.25`（EPSG4326 整数 zoom 粒度粗，细粒度让 fitBounds 更准）；**自定义交互层**通过 `window.TeachAnyMaps['<map-id>']` 获取 map 实例后 `L.xxx.addTo(map)`（如 geo-monsoon 的季风区/箭头叠加），脚本会等 Leaflet 异步加载就绪（最多10秒）。参考实现：`community/geo-m-china-transportation/`（中国聚焦）、`community/geo-monsoon/`（世界+交互层叠加）、`community/hist-m-renaissance/`（欧洲+6城）。
